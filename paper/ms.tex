...
\begin{abstract}
    ...
    occurred $81.5 \pm 11.8$~seconds earlier than had been predicted,
    ...
    6.3$\sigma$ level.  The 1.3-day orbital period of WASP-4b appears
...
% JNW: Luke, didn't you also have to fit a linear trend for each transit?
% Otherwise the fit would be poor, for each transit, even if the
% trends tend to cancel out in the phase folded light curve.
% And, did you assign the photometric uncertainties as described below?
...
\end{figure*}
% JNW: I think the KDE needs to be removed and perhaps replaced with something
% else.  It gives the impression that the WASP-4 anomaly is only at
% the 2-sigma level, which is false.  The KDE illustrates the spread
% in values, but what is relevant is the standard deviation of the
% mean of the measurements, which is narrower than the KDE implies.
% I suggest replacing it with a Gaussian with a mean equal to the mean of the
% deviations, and a width equal to the uncertainty in the mean, i.e., the
% standard deviation divided by sqrt(N-1).
