...
% % The HJ semimajor axis distributions and rotation period distribn
% % indicate that tidal decay is real.  (So too, might the age
% % distribution.)
% Stars hosting hot Jupiters tend to spin faster than other stars of
% similar age and mass \citep{penev_hats-18_2016,penev_empirical_2018}.
% This is expected because when the orbital frequency is larger than the
% spin rotation frequency, tides donate angular momentum from the planet
% to the star.
% The reverse process occurs between the Earth and Moon, leading to the
% Moon's slowly increasing separation from Earth of $X.XX$cm~yr$^{-1}$
% (CITE lunar ranging experiment).
% 
% The hot Jupiter star-planet separation distribution is best reproduced
% by models that account for tides~\citep{cameron_hierarchical_2018}.


% There have been some indications of period timing changes for hot jupiters
...
% Both \citet{maciejewski_departure_2016} and \citet{patra_2017}
% emphasized that the timing variations could be caused by , or
% also by tidal decay.
...
% % There are also indications that apsidal precession should happen. If
% % there are any trace remnants of eccentricity, this will produce
% % timing variations.
% Timing variations can also be caused by rotation of
% the orbital ellipse within the orbital plane (apsidal precession).
% This happens because any rotational or tidal bulge creates a
% gravitation quadrupole field ($r^{-3}$, rather than the typical
% $r^{-2}$), which causes the line of apsides to move forward or back.
% \citet{ragozzine_probing_2009} examined apsidal precession.


% Tidal decay of hot Jupiters has been invoked to explain certain
% properties of the ensemble of star-planet systems. For example, the
% scarcity of gas giants with periods less than a day is suggestive of
% orbital decay \citep[see,
%   e.g.][]{Jackson+2008,Hansen2010,Penev+2012,Ogilvie2014}. The
% anomalously rapid rotation of some hot-Jupiter host stars has been
% attributed to transfer of the planet's orbital angular momentum
% \citep{Penev+2016}. The absence of hot Jupiters around subgiant stars
% may be caused by an acceleration of orbital decay when a star leaves
% the main sequence
% \citep{VillaverLivio2009,Hansen2010,SchlaufmanWinn2013}.  Tidal decay
% might also be responsible for the lower occurrence of close-in planets
% around rapidly rotating stars \citep{TeitlerKonigl2014}, or the
% realignment of stars and their planetary orbits
% \citep{MatsakosKonigl2015}. However, direct evidence for orbital decay
% has been lacking: there have been no clear demonstrations of a
% long-term period decrease due to orbital decay \citep[see,
%   e.g.,][]{Hoyer+2016,Wilkins+2017}.
% 
% Another unfulfilled prediction is that the orbits of hot Jupiters
% should be apsidally precessing on a timescale of decades
% \citep{MiraldaEscude2002,HeylGladman2007,PalKocsis2008,JordanBakos2008},
% as long as the orbits are at least slightly eccentric. In particular,
% \citet{RagozzineWolf2009} noted that the theoretical precession rate
% is dominated by the contribution from the planet's tidally deformed
% mass distribution. They advocated a search for apsidal precession as
% a means of probing the interiors of hot Jupiters.
% 
...
%at Las Campanas\footnote{\citet{winn_transit_2009} recorded the GPS clock
%	time in UT reference. They then converted this to BJD in the TDB
%	reference for their reported midtimes.},
%and reported transit midtimes precise to
%6~seconds.  \citet{dragomir_terms_2011}
%updated the ephemeris using two transits measured with the CTIO 0.9-m
%and 1.0-m.  \citet{sanchis-ojeda_starspots_2011} reported four more
%transits from
%the Baade telescope at Las Campanas\footnote{
...
%We consider the effects of including ETD data in Appendix~\ref{sec:verify_archival_times}.
...
%, the travelling equilibrium tidal
%bulge is damped by turbulent viscosity in the convective envelope.
...
% 0.0018$. 
% \citet{zhou_secondary_2015} report a weaker $e\cos\omega = -0.001
% \pm 0.003$. 
...
%FIXME: take a look at RVs. rule this shit out. /  understand where
%the bounds are.
